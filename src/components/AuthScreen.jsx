...
export default function AuthScreen({ onLogin }) {
  ...
  const styles = {
    ...
    forgot: { textAlign: 'right', fontSize: '0.78rem', color: 'var(--gold)', cursor: 'pointer', marginTop: '-6px' },
    note: { textAlign: 'center', fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '16px' },
    link: { color: 'var(--gold)', textDecoration: 'none' },
    back: {
      background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer',
      fontSize: '0.85rem', fontFamily: 'var(--sans)', marginTop: '8px', alignSelf: 'center'
    }
  }
        ...
        {tab === 'login' ? (
          <form onSubmit={handleLogin} style={styles.form}>
            ...
            <div style={styles.inputWrap}>
              <input className="input" type={showLoginPwd ? 'text' : 'password'} placeholder="Sua senha" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} autoComplete="current-password" required style={{ paddingRight: '44px', width: '100%' }} />
              {eyeBtn(showLoginPwd, () => setShowLoginPwd(!showLoginPwd))}
            </div>
          ...
          <form onSubmit={handleRegister} style={styles.form}>
            ...
            <div style={styles.inputWrap}>
              <input className="input" type={showRegPwd ? 'text' : 'password'} placeholder="Crie uma senha (mín. 6 caracteres)" value={regPassword} onChange={e => setRegPassword(e.target.value)} autoComplete="new-password" required style={{ paddingRight: '44px', width: '100%' }} />
              {eyeBtn(showRegPwd, () => setShowRegPwd(!showRegPwd))}
            </div>
            {error && <div className="error-box">{error}</div>}
            <button type="submit" className="btn-primary" style={{ width: '100%' }} disabled={loading}>
