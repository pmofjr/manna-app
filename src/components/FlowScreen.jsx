...
export default function FlowScreen({ trilha, user, onGenerate, onBack, onHistory, onLogout }) {
  ...
  function handleCause(skip = false) {
    onGenerate({
      emotion: emotion.id,
      cause: skip ? null : (cause.trim() || null),
      blessName: blessName.trim() || null
    })
  }
  ...
  if (step === 'cause') return (
    <div style={containerStyle}>
      ...
      <div style={innerStyle}>
        ...
        {stepTitle(
          trilha === 'outro'
            ? `Quer contar um pouco mais sobre o que<br/><em style="font-style:italic;color:var(--gold)">${targetFirstName}</em> está sentindo?`
            : `Quer contar um pouco mais sobre<br/>o que está <em style="font-style:italic;color:var(--gold)">sentindo</em>?`
        )}
        <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)', marginTop: '-8px' }}>
          Opcional. Quanto mais você compartilhar, mais personalizada será a palavra.
        </p>
        <textarea
          className="input"
          placeholder={
            trilha === 'outro'
              ? `Descreva brevemente o que ${targetFirstName} está vivendo...`
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          <button
            className="btn-primary"
            onClick={() => handleCause(false)}
            style={{ alignSelf: 'flex-start' }}
          >
            Gerar palavra →
          </button>
          <button
            onClick={() => handleCause(true)}
            style={{
              background: 'none', border: 'none',
              color: 'var(--text-muted)', cursor: 'pointer',
              fontSize: '0.85rem', fontFamily: 'var(--sans)',
